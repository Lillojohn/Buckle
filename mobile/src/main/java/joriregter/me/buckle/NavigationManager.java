...
public class NavigationManager extends Observable implements IALocationListener, SensorEventListener {
    ...
    public NavigationManager (SensorManager sensorManager, IALocationManager indoorAtlasLocMan, LatLng defaultLocation) {
        updateNextWaypoint();
    ...
    public static NavigationManager getInstance (SensorManager sensorManager, IALocationManager indoorAtlasLocMan) {
        if (_nvm == null) {
            _nvm = new NavigationManager(sensorManager, indoorAtlasLocMan, new LatLng(51.917437458857066, 4.484745936568862));
        }
    ...
    public boolean atWaypoint(LatLng location, LatLng waypoint, double proximity) {
        LatLngBounds vicinity = new LatLngBounds(
                new LatLng(
                        location.latitude - proximity,
                        location.longitude - proximity
                ),
                new LatLng (
                        location.latitude + proximity,
                        location.longitude + proximity
